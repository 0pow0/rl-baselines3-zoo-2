...
def evaluate(model: nn.Module, loader: DataLoader, device: th.device) -> tuple[float, float]:
    model.eval()
    total_loss = 0.0
    count = 0
...
def main() -> None:
    ...
    if val_loader is not None:
        final_val_loss, final_val_corr = evaluate(model, val_loader, device)
